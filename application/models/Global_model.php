	...
	function search(){	
		...
		else
		{
			...
			if($offset_before == 0){
				...
				ORDER BY `car_status` DESC ) AS car ORDER BY ".$field." ".$order_by." 
				");
				$num_rows = $n->num_rows();
			}
			$query = "
			...
			SELECT car_uid, cb_uid, cm_uid, car_color, car_model_year, album_uid, ".$field.", car_in_stock, car_status 
			  FROM cars
			WHERE car_model_year >= ".$year_from." AND car_model_year <= ".$year_to." ".$where." GROUP BY `car_link`, `".$field."`, `car_color`, `car_status` ORDER BY `car_status` DESC LIMIT 15 OFFSET ".$offset."
			) AS car ORDER BY ".$field." ".$order_by." 
			";
			$q = $this->db->query($query);
